...
export function ContentRequirementsScreen({ setActiveTab }: ContentRequirementsScreenProps) {
  ...
  const contentTypes: ContentType[] = [
    { id: "video", name: "Video", description: "Reels, Shorts", icon: "star" },
    { id: "photo", name: "Photo", description: "Feed posts, carousels", icon: "camera" },
    { id: "stories", name: "Stories", description: "Temporary content", icon: "check" },
    { id: "live", name: "Live", description: "Live broadcasts", icon: "plus" },
  ];

  const platforms: Platform[] = [
    { id: "instagram", name: "Instagram", icon: "camera" },
    { id: "tiktok", name: "TikTok", icon: "star" },
    { id: "x", name: "X", icon: "star" },
  ];
  ...
  const toggleContentType = (contentTypeId: string) => {
    setSelectedContentTypes(prev => prev.includes(contentTypeId) ? prev.filter(id => id !== contentTypeId) : [...prev, contentTypeId]);
  };

  const togglePlatform = (platformId: string) => {
    // Only Instagram is enabled for now
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex justify-between items-center">
        <button onClick={handleBack} className="text-blue-500 hover:text-blue-600 font-medium">
          ← Back
        </button>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {contentTypes.map((contentType) => (
            <Card
              key={contentType.id}
              className={`p-4 cursor-pointer transition-all ${selectedContentTypes.includes(contentType.id) ? "border-2 border-blue-500 bg-blue-50" : "border border-gray-200 bg-white hover:border-gray-300"}`}
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {platforms.map((platform) => {
            ...
            const baseClasses = isInstagram
              ? (isSelected ? "border-2 border-purple-500 bg-purple-50" : "border border-gray-200 bg-white hover:border-gray-300")
              : "border border-gray-200 bg-white opacity-60 cursor-not-allowed";
            return (
              <Card
                key={platform.id}
                className={`p-4 transition-all ${baseClasses}`}
      ...
      <div className="flex space-x-3">
        <Button onClick={handleBack} variant="outline" className="flex-1 bg-white border-gray-300 text-gray-700 hover:bg-gray-50 py-3 rounded-lg font-medium">
          <Icon name="arrow-right" className="mr-2 rotate-180" />
          Back
        </Button>
        <Button onClick={handleContinue} disabled={!isFormValid} className="flex-1 bg-blue-500 hover:bg-blue-600 text-white py-3 rounded-lg font-medium disabled:bg-gray-300 disabled:cursor-not-allowed">
          Continue
